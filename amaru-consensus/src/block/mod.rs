...
use malachite_base::num::arithmetic::traits::{Ceiling, Pow};
use pallas_crypto::hash::{Hash, Hasher};
use pallas_crypto::vrf::vrf_verify;
...
impl<'b> BlockValidator<'b> {
    ...
    fn validate_babbage_compatible(&self) -> bool {
        ...
        match vrf_verify(vrf_vkey, block_vrf_proof, vrf_input_seed.as_ref()) {
            Ok(proof_hash) => {
    ...
    fn pool_meets_delegation_threshold(
        ...
        let limbs = leader_vrf_output
            ...
            .collect();
        let certified_leader_vrf = FixedDecimal::from(
            Natural::from_owned_limbs_desc(limbs) * Natural::from(10u64).pow(34),
        );
    ...
    fn mk_vrf_input(&self, slot_in_epoch: u64, eta0: &[u8]) -> Hash<32> {
        trace!("mk_vrf_input() slot_in_epoch {}", slot_in_epoch);
        let mut hasher = Hasher::<256>::new();
        hasher.input(&slot_in_epoch.to_be_bytes());
        hasher.input(eta0);
        hasher.finalize()
    }
