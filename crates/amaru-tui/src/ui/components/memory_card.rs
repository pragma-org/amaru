...
pub(in crate::ui) fn render_memory_card(
    ...
    render_memory_row(frame, rows[0], "Footprint", sample.map(|sample| sample.process_memory_bytes), sample, mode, false);
...
fn render_memory_row(
    ...
    frame.render_widget(
        Paragraph::new(label)
            .style(if dimmed { muted() } else { Style::default() })
...
fn memory_ratio(current_bytes: u64, total_bytes: u64) -> f64 {
    if total_bytes == 0 {
        ...
        (current_bytes as f64 / total_bytes as f64).clamp(0.0, 1.0)
    }
}
