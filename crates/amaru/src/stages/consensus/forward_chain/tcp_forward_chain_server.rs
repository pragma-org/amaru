...
impl TcpForwardChainServer {
    ...
    pub fn new(
        ...
        // Start a TCP listener synchronously but in non-blocking mode
        // so we can integrate it into the Tokio runtime later.
        // This allows us to create the TcpForwardChainServer in a synchronous context.
        let std_listener = std::net::TcpListener::bind(&listen_address)?;
        std_listener.set_nonblocking(true)?;

        let tcp_listener = runtime
            .with_rt(|rt| {
                let _e = rt.enter(); // attach to that runtime
                tokio::net::TcpListener::from_std(std_listener)
                    .expect("failed to wrap std listener")
            })
            .unwrap();

        TcpForwardChainServer::create(
            store,
            Arc::new(runtime),
            tcp_listener,
            network_magic,
