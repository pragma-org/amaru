...
async fn test_chain_sync() {
    ...
    let (point, tip) = client.find_intersect(vec![to_network_point(header142.point())]).await;

    let lost = chain_forwarder.store.load_header(&hash(LOST_47)).unwrap().clone();
    ...
    assert_eq!(headers, vec![ClientMsg::Backward(to_network_point(header142.point()), to_pallas_tip(lost.as_tip()))]);

    chain_forwarder.send_backward(FORK_47).await;
    chain_forwarder.send_forward(WINNER_47).await;
    chain_forwarder.send_forward(&chain[8].hash().to_string()).await;
    ...
    assert_eq!(
        ...
        ClientMsg::Backward(to_network_point(header142.point()), to_pallas_tip(chain[6].as_tip()))
    );
...
async fn test_sync_optimising_rollback() {
    ...
    client.find_intersect(vec![]).await.0.expect("no intersection");
    ...
    assert_eq!(
        msgs,
        [
            ClientMsg::Backward(to_network_point(Point::Origin), to_pallas_tip(lost.as_tip())),
