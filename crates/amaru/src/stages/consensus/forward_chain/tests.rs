...
#[test]
fn test_chain_sync() {
    let mut setup = Setup::new(LOST_47).unwrap();
    ...
    let (point, tip) = client.find_intersect(vec![chain[6].pallas_point()]);
    ...
    setup.send_forward(WINNER_47);
    setup.send_forward(&chain[8].hash().to_string());
...
fn test_sync_optimising_rollback() {
    ...
    setup.send_backward(BRANCH_47);
    setup.send_forward(&chain[7].hash().to_string());
    setup.send_forward(&chain[8].hash().to_string());
    setup.send_forward(&chain[9].hash().to_string());
