...
impl Setup {
    pub fn new(our_tip: &str) -> anyhow::Result<Setup> {
        ...
        let runtime = Arc::new(AcTokio::new("test", 1)?);
        let header = store.load_header(&hash(our_tip)).unwrap();

        let tcp_listener = block_on(runtime.clone(), async move {
    ...
    pub fn runtime(&self) -> Arc<AcTokio> {
        self.runtime.clone()
    }

    pub fn send_forward(&mut self, s: &str) {
        let header = self.store.load_header(&hash(s)).unwrap();
        tracing::info!("sending forward event");
        block_on(
            self.runtime(),
            self.listener.send(ForwardEvent::Forward(header)),
        )
        .unwrap();
    }
