...
use amaru_consensus::consensus::{
    ChainSyncEvent, fetch_block, receive_header, select_chain, store_block, store_header,
    validate_header,
};
use amaru_kernel::block::{BlockValidationResult, StageError};
use amaru_kernel::protocol_parameters::GlobalParameters;
use amaru_ledger::block_validator;
...
pub fn build_stage_graph(
    ...
    let validate_block_stage = network.stage("validate_block", block_validator::stage);

    // TODO: currently only validate_header errors, will need to grow into all error handling
    let upstream_errors_stage =
        network.stage("upstream_errors", async |_, error: StageError, eff| {
