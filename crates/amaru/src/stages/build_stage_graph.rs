...
pub fn build_stage_graph(
    ...
    let receive_header_stage = network.stage("receive_header", with_consensus_effects(receive_header::stage));
    let validate_header_stage = network.stage("validate_header", with_consensus_effects(validate_header::stage));
    let fetch_block_stage = network.stage("fetch_block", with_consensus_effects(fetch_block::stage));
    let validate_block_stage = network.stage("validate_block", with_consensus_effects(validate_block::stage));
    let select_chain_stage = network.stage("select_chain", with_consensus_effects(select_chain::stage));
    let forward_chain_stage = network.stage("forward_chain", with_consensus_effects(forward_chain::stage));

    let validation_errors_stage = network.stage("validation_errors", async move |manager, error, eff| {
    ...
    let processing_errors_stage = network.stage("processing_errors", async |_, error: ProcessingFailed, eff| {
    ...
    let forward_chain_stage = network.wire_up(
        forward_chain_stage,
        (our_tip, validation_errors_stage.clone().without_state(), processing_errors_stage.clone().without_state()),
    );

    let select_chain_stage = network.wire_up(
        select_chain_stage,
        (chain_selector, forward_chain_stage.without_state(), validation_errors_stage.clone().without_state()),
    );
    ...
    let validate_header_stage = network.wire_up(
        ...
        (fetch_block_stage.without_state(), validation_errors_stage.clone().without_state()),
    );
