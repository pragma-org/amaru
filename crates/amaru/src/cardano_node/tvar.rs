...
mod tests {
    ...
    use amaru_kernel::{
        EraHistory, Hash, NetworkName, PREPROD_DEFAULT_PROTOCOL_PARAMETERS, Point, TransactionInput, Value, cbor,
    };
    use amaru_ledger::store::ReadStore;
    use amaru_progress_bar::no_progress_bar;
    use amaru_stores::in_memory::MemoryStore;
    use tempfile::NamedTempFile;

    use super::import_utxo_from_tvar;

    const SYNTHETIC_TVAR: &[u8] = include_bytes!("test_data/synthetic.tvar");

    #[test]
    fn imports_synthetic_tvar_preserving_legacy_and_modern_shapes() {
        let mut file = NamedTempFile::new().unwrap();
        file.write_all(SYNTHETIC_TVAR).unwrap();
        file.as_file_mut().seek(SeekFrom::Start(0)).unwrap();

        let era_history = <&EraHistory>::from(NetworkName::Preprod).clone();
        let db = MemoryStore::new(era_history.clone(), (*PREPROD_DEFAULT_PROTOCOL_PARAMETERS).clone());
        let point = Point::Specific(1.into(), Hash::new([0x33; 32]));

        import_utxo_from_tvar(file.as_file_mut(), &db, no_progress_bar, &point, &era_history, NetworkName::Preprod)
            .unwrap();

        let utxos = db.iter_utxos().unwrap().collect::<BTreeMap<_, _>>();
    ...
    fn transaction_input(byte: u8, index: u16) -> TransactionInput {
        TransactionInput { transaction_id: Hash::new([byte; 32]), index: index.into() }
    }
}
