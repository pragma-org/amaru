...
Use `make help` from one of those network directories to see the available utilities. They cover local fixture compression, optional local tarball creation, plus listing, downloading, and uploading the corresponding S3 bucket contents. The S3 helpers work on individual `epoch_*.json.zst` objects and only transfer files that are missing locally or remotely.
