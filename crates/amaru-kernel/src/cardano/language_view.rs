...
use crate::{CostModels, PlutusVersion, cbor};
...
impl LanguageView {
    ...
    pub fn from_cost_models(language: PlutusVersion, cost_models: &CostModels) -> Self {
        let cost_model = match language {
            PlutusVersion::V1 => cost_models.plutus_v1.clone(),
            PlutusVersion::V2 => cost_models.plutus_v2.clone(),
            PlutusVersion::V3 => cost_models.plutus_v3.clone(),
        };
    ...
    ///
    /// **Plutus V2 (language id 1) and Plutus V3 (language id 2):**
    ...
    ///
    /// This means Plutus V2 (tag `0x01`, 1 byte) sorts before Plutus V1 (tag `0x41 0x00`, 2 bytes).
    ///
    ...
    fn encoded_tag(&self) -> &'static [u8] {
        match self.language {
            PlutusVersion::V1 => &[0x41, 0x00],
            PlutusVersion::V2 => &[0x01],
            PlutusVersion::V3 => &[0x02],
        }
    }
}
...
impl<C> cbor::Encode<C> for LanguageView {
    fn encode<W: cbor::encode::Write>(
        ...
        match (&self.language, self.cost_model.as_ref()) {
            ...
            (PlutusVersion::V1, None) => {
                e.bytes(&[0xf6])?;
            }
            (PlutusVersion::V2 | PlutusVersion::V3, Some(params)) => {
                e.array(params.len() as u64)?;
                for &param in params {
                    e.i64(param)?;
                }
            }
            (PlutusVersion::V2 | PlutusVersion::V3, None) => {
                e.null()?;
            }
        };
...
mod tests {
    ...
    #[test_case(V1, &[0x41, 0x00, 0x41, 0xf6] ; "v1 null params")]
    #[test_case(V2, &[0x01, 0xf6] ; "v2 null params")]
    #[test_case(V3, &[0x02, 0xf6] ; "v3 null params")]
    fn language_view_encodes_null_when_cost_model_missing(language: PlutusVersion, expected: &[u8]) {
