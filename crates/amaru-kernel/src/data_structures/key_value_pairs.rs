...
impl<K: Ord, V> From<KeyValuePairs<K, V>> for BTreeMap<K, V> {
    fn from(set: KeyValuePairs<K, V>) -> Self {
        BTreeMap::from_iter(Vec::from(set))
    }
}

impl<K: Eq, V> From<BTreeMap<K, V>> for KeyValuePairs<K, V> {
    fn from(entries: BTreeMap<K, V>) -> Self {
        Self(entries.into_iter().collect::<Vec<_>>())
    }
}

impl<K: Eq, V> TryFrom<Vec<(K, V)>> for KeyValuePairs<K, V> {
...
impl<K: Eq, V> Deref for KeyValuePairs<K, V> {
    type Target = [(K, V)];
...
impl<K: Eq, V> AsRef<[(K, V)]> for KeyValuePairs<K, V> {
    fn as_ref(&self) -> &[(K, V)] {
        self.0.deref()
    }
}

impl<K: Eq, V> IntoIterator for KeyValuePairs<K, V> {
    type Item = (K, V);
...
// ----------------------------------------------------------------------------
// IntoKeyValuePairsError
// ----------------------------------------------------------------------------

#[derive(Debug, thiserror::Error)]
pub enum IntoKeyValuePairsError {
...
mod tests {
    ...
    proptest! {
        #[test]
        fn roundtrip_encode_decode(elems in collection::vec((any::<u8>(), any::<u8>()), 0..30)) {
            let set: Vec<(u8, u8)> = BTreeMap::from_iter(elems).into_iter().collect();
            let kvs: KeyValuePairs<u8, u8> = KeyValuePairs::try_from(set).unwrap();
            assert_eq!(
                from_cbor_no_leftovers::<KeyValuePairs<u8, u8>>(to_cbor(&kvs).as_slice()).unwrap(),
                kvs,
            )
        }
    }

    #[test_case("A0", &[], true; "empty map")]
