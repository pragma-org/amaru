    ...
    for entry in read_directory(rule_dir)? {
        ...
        for file in read_directory(&dir)? {
            ...
            if path.extension().is_some_and(|ext| ext == "cbor") {
                out.push(TestKey::new(corpus, rule.clone(), category.clone(), path));
            }
        }
    }
    out.sort();
...
pub fn read_expected_canonical_cbor(corpus: Corpus, rule: &str, path: &PathBuf) -> anyhow::Result<Option<Vec<u8>>> {
