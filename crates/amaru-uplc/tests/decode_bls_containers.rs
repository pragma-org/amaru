...
fn assert_decodes(flat: &[u8], description: &str) {
    let arena = Arena::new();
    let (_, remainder) = decode::<DeBruijn>(&arena, flat, PlutusVersion::V3, ProtocolVersion::new(11, 0))
...
fn assert_fails_on_bls_value(flat: &[u8], description: &str) {
    let arena = Arena::new();
    let error = decode::<DeBruijn>(&arena, flat, PlutusVersion::V3, ProtocolVersion::new(11, 0))
