        ...
        match msg {
            Tracked::Wrapped(ChainSyncEvent::RollForward {
                ...
                let header = match decode_header(&point, raw_header.as_slice()) {
                    Ok(header) => header,
                    Err(error) => {
                    tracing::error!(%error, %point, %peer, "receive_header.decode_failed");
                        eff.base()
                ...
                tracing::error!(%point, %peer, header_point=%header.point(), "receive_header.point_mismatch");
...
mod tests {
    ...
    #[tokio::test]
    async fn caughtup_is_sent_to_downstream() -> anyhow::Result<()> {
        let message = Tracked::CaughtUp {
            peer: Peer::new("name"),
            span: Span::current(),
        };
        let consensus_ops = mock_consensus_ops();
