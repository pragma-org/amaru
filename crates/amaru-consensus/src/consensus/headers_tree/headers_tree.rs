...
impl<H: IsHeader + Clone + Debug + PartialEq + Eq + Send + Sync + 'static> HeadersTree<H> {
    ...
    fn create(chain_store: Arc<dyn ChainStore<H>>, max_length: usize) -> HeadersTree<H> {
        ...
        peers.insert(Me, vec![chain_store.get_anchor_hash()]);
    ...
    fn insert_header(&mut self, header: &H) -> Result<(), ConsensusError> {
        ...
        if header.parent().is_none() {
            if self.is_empty_tree() {
                ...
                self.set_best_chain(&header.hash())?;
...
impl<H: IsHeader + Clone + Debug + PartialEq + Eq> HeadersTree<H> {
    ...
    /// Store the best currently known tip
    fn set_best_chain(&self, hash: &HeaderHash) -> Result<(), ConsensusError> {
        self.chain_store
...
mod tests {
    ...
    fn initialize_peer_on_empty_tree() {
        let mut tree: HeadersTree<TestHeader> = HeadersTree::new_in_memory(10);
