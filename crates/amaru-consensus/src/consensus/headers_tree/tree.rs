...
impl<H> Tree<H> {
    ...
    fn pretty_print_with_prefix_and_format(&self, prefix: &str, is_last: bool, format: fn(&H) -> String, out: &mut String) {
        out.push_str(prefix);
        if !prefix.is_empty() {
...
impl<H: IsHeader + Clone + Debug + PartialEq + Eq> Tree<H> {
    ...
    pub fn add(&mut self, parent_hash: Hash<HEADER_HASH_SIZE>, new: &H) -> bool {
    ...
    pub fn hashes(&self) -> Vec<Hash<HEADER_HASH_SIZE>> {
...
impl<H: IsHeader + Clone + Debug + PartialEq + Eq> Tree<H> {
    pub fn to_map(&self) -> BTreeMap<Hash<HEADER_HASH_SIZE>, H> {
        let mut map = BTreeMap::new();
        self.to_map_recursive(&mut map);
        map
    }

    fn to_map_recursive(&self, map: &mut BTreeMap<Hash<HEADER_HASH_SIZE>, H>) {
        map.insert(self.value.hash(), self.value.clone());
        for child in &self.children {
            child.to_map_recursive(map);
        }
    }
    pub fn from(headers: &BTreeMap<Hash<HEADER_HASH_SIZE>, H>) -> Self {
        let root = headers.values().find(|h| h.parent().is_none()).or_else(|| headers.values().find(|h| !headers.contains_key(&h.parent().unwrap()))).unwrap();
        let mut headers_clone = headers.clone();
        headers_clone.remove(&root.hash());
        Self::make_tree_from(root.clone(), &mut headers_clone)
    }

    fn make_tree_from(root: H, headers: &mut BTreeMap<Hash<HEADER_HASH_SIZE>, H>) -> Tree<H> {
        let mut tree = Tree::make_leaf(&root);
        let roots: Vec<H> = headers.values().filter(|n| headers.get(&n.parent().unwrap()).is_none()).map(|h| h.clone()).collect::<Vec<_>>();
        let mut children = vec![];
        for root in roots.iter() {
            headers.remove(&root.hash());
        }
        for root in roots.into_iter() {
            children.push(Self::make_tree_from(root.clone(), headers))
        }
...
#[cfg(test)]
mod tests {
    use crate::consensus::headers_tree::data_generation::{generate_headers_chain, generate_test_header_tree};
    use super::*;

    #[test]
    fn test_creation_from_map() {
        let tree = generate_test_header_tree(4, 42);
        assert_eq!(Tree::from(&tree.to_map()), tree);
    }
