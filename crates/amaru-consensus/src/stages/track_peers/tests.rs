...
fn test_roll_forward_transient_validation_failure_keeps_peer() {
    ...
    let (_running, _guards, mut logs) = setup_with_validation(
        &prep.rt_handle(),
        state,
        msg,
        build_store(&[]),
    ...
    // Transient failure should produce a WARN log with the pending-stake-distribution event;
    // no ERROR (the peer must not be dropped) and no peer-removal RPC.
    logs.assert_and_remove(Level::WARN, &["chain_sync.validate_header.pending_stake_distribution"])
        .assert_no_remaining_at([Level::ERROR]);
}
