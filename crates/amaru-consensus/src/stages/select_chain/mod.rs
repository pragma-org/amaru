...
// Re-export under the historical name used by track_peers / call sites.
pub use crate::performance::HeaderLifecycleOutcome as PerfHeaderForwardOutcome;
...
pub enum SelectChainMsg {
    ...
    TipFromUpstream { tip: Tip, parent: Point, trace_context: TraceContext },
...
impl SelectChain {
    ...
    async fn handle_block_validation_result(
        ...
        if valid {
            ...
            let now = eff.clock().await;
            eff.external(Performance::record_block_valid(h, now)).await;
            return;
        }
        // INVALID CASE
        //
        ...
        let now = eff.clock().await;
        for hash in &pruned {
            eff.external(Performance::record_block_pruned(*hash, hash == &tip.hash(), now)).await;
        }
