...
use amaru_protocols::store_effects::{FindRollbackPointEffect, LoadBlockEffect, LoadHeaderEffect, ResourceHeaderStore};
...
pub fn te_find_rollback_point(at_stage: &str, parent: Point, ledger_tip: Point) -> TraceMatch<'static> {
    TraceEntry::suspend(Effect::external(at_stage, Box::new(FindRollbackPointEffect::new(parent.hash(), ledger_tip))))
        .into()
}
