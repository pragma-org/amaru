...
impl SelectChain {
    ...
    fn switch_to_fork(
        ...
        let mut result = vec![ValidateHeaderEvent::Rollback { rollback_point, peer: peer.clone(), span: span.clone() }];

        for header in fork {
            result.push(SelectChain::forward_block(peer.clone(), header, span.clone()));
        }
    ...
    pub async fn select_roll_forward(
        ...
        let events = match result? {
            ...
            ForwardChainSelection::SwitchToFork(Fork { peer, rollback_header, fork }) => {
    ...
    pub async fn select_rollback(
        ...
        match result {
            RollbackChainSelection::SwitchToFork(Fork { peer, rollback_header, fork }) => {
                info!(rollback_point = %rollback_header.point(), length = fork.len(), "rollback.switch_to_fork");
                Ok(SelectChain::switch_to_fork(peer, rollback_header.point(), fork, span))
            }
            ...
            RollbackChainSelection::RollbackBeyondLimit { peer, rollback_point, max_point } => {
                Err(ConsensusError::InvalidRollback { peer, rollback_point, max_point })
            }
        }
    }
        ...
        Box::pin(async move {
            match chain_sync {
                DecodedChainSyncEvent::RollForward { peer, header, span, .. } => {
                    self.select_roll_forward(store.clone(), peer, header, span).await
                }
                DecodedChainSyncEvent::Rollback { peer, rollback_point, span } => {
                    self.select_rollback(store.clone(), peer, rollback_point, span).await
                }
            }
        })
    }
...
type State = (SelectChain, StageRef<BlockValidationResult>, StageRef<ValidationFailed>);
...
pub fn stage(
    ...
    async move {
        ...
        let events = match select_chain.handle_chain_sync(store.clone(), msg).await {
            Ok(events) => events,
            Err(e) => {
                eff.base().send(&errors, ValidationFailed::new(&peer, e)).await;
                ...
                ValidateHeaderEvent::Rollback { peer, rollback_point, span } => {
                    match eff.store().load_header(&rollback_point.hash()) {
                        Some(rollback_header) => match store.rollback_chain(&rollback_header.point()) {
                            Ok(_size) => {
                                let event = BlockValidationResult::RolledBackTo { peer, rollback_header, span };
                                eff.base().send(&downstream, event).await;
                            }
                            Err(e) => {
                                eff.base()
                                    .send(
                                        ...
                                        ValidationFailed {
                                            peer,
                                            error: ConsensusError::RollbackChainFailed(rollback_header.point(), e),
                                        },
                                    )
                                    .await
                            }
...
mod tests {
    ...
    async fn a_roll_forward_updates_the_tree_state() -> anyhow::Result<()> {
        ...
        assert_eq!(
            ...
            BTreeMap::from_iter(vec![("downstream".to_string(), vec![format!("{:?}", output)])])
        );

        check_peers(
            select_chain,
            vec![(Me, vec![header0.hash(), header1.hash()]), (SomePeer(peer), vec![header0.hash(), header1.hash()])],
        );
        Ok(())
    }
    ...
    async fn an_error_does_not_reset_the_tree_state() -> anyhow::Result<()> {
        ...
        assert_eq!(state3.0.tree_state, state2.0.tree_state, "the tree state should not change after an error");
        ...
        assert_eq!(
            consensus_ops.mock_base.received(),
            BTreeMap::from_iter(vec![
                ("downstream".to_string(), vec![format!("{:?}", output1), format!("{:?}", output2)]),
        ...
        check_peers(
            ...
            vec![
                (Me, vec![header0.hash(), header1.hash(), header2.hash()]),
                (SomePeer(peer1), vec![header0.hash(), header1.hash(), header2.hash()]),
    ...
    fn make_state(store: Arc<dyn ChainStore<BlockHeader>>, peers: &[&Peer], anchor: &HeaderHash) -> State {
        ...
        for peer in peers {
            tree_state.initialize_peer(store.clone(), peer, anchor).unwrap();
        }
    ...
    fn make_roll_forward_message(peer: &Peer, header: &BlockHeader) -> DecodedChainSyncEvent {
        DecodedChainSyncEvent::RollForward { peer: peer.clone(), header: header.clone(), span: Span::current() }
    }

    fn make_rollback_message(peer: &Peer, header: &BlockHeader) -> DecodedChainSyncEvent {
        DecodedChainSyncEvent::Rollback { peer: peer.clone(), rollback_point: header.point(), span: Span::current() }
    }

    fn make_block_validated_event(peer: &Peer, header: &BlockHeader) -> BlockValidationResult {
        BlockValidationResult::BlockValidated { peer: peer.clone(), header: header.clone(), span: Span::current() }
    }

    fn check_peers(select_chain: SelectChain, expected: Vec<(Tracker, Vec<HeaderHash>)>) {
        let actual = select_chain.tree_state.peers().iter().map(|(k, vs)| (k.clone(), vs.clone())).collect::<Vec<_>>();
