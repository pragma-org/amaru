...
///
/// Slots are assigned close to block height while remaining strictly increasing along each branch,
/// so 2 different forks at the same depth can produce nearby but distinct slots.
///
...
/// Generate a chain of headers anchored at a given header, with slots varying around block height.
fn generate_headers(
    length: usize,
    start_block: u64,
    start_slot: u64,
    ...
    let mut previous_slot = start_slot;

    for i in 1..=length {
        let block_number = start_block + i as u64;
        let expected_slot = block_number;
        let candidate_slot = rng.random_range(expected_slot.saturating_sub(SLOT_JITTER)..=expected_slot + SLOT_JITTER);
        let slot = candidate_slot.max(previous_slot.saturating_add(1));
...
mod tests {
    ...
    fn check_nodes(node: &Tree<BlockHeader>) {
        assert!(node.value.slot().as_u64().abs_diff(node.value.header_body().block_number) <= SLOT_JITTER);
        ...
        for child in &node.children {
            ...
            assert!(child.value.slot() > node.value.slot());
            check_nodes(child);
        }
    }
}
