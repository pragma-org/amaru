...
impl RunConfig {
    ...
    pub fn upstream_peers(&self) -> Vec<Peer> {
        (1..=self.number_of_upstream_peers as u16).map(|i| Peer::for_test(30000 + i)).collect()
    }

    pub fn downstream_peers(&self) -> Vec<Peer> {
        (1..=self.number_of_downstream_peers as u16).map(|i| Peer::for_test(40000 + i)).collect()
    }
