...
pub async fn stage(mut state: State, msg: MuxMessage, mut eff: Effects<MuxMessage>) -> State {
    ...
    handle_msg(msg, &eff, muxer, sending, writer, reader)
        ...
        .or_terminate(&eff, async |error| {
            ...
            error!(protocols::mux::FAILED, peer = &peer, role = muxer.role().to_string(), operation = "muxing", error = err.to_string());
        })
        .await;
...
async fn handle_msg(
    ...
    match msg {
        ...
        MuxMessage::Terminate => {
            debug!(protocols::mux::TERMINATING, role = muxer.role().to_string());
