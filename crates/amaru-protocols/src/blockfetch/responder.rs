...
pub mod tests {
    ...
    use amaru_kernel::{
        BlockHeight, EraHistory, EraName, IsHeader, NetworkPoint, NonEmptyBytes, Slot, any_header, any_headers_chain,
    ...
    fn test_request_range_no_parent_hash_before_from() {
        let genesis = NetworkPoint::Specific(Slot::from(10), run_strategy(any_header()).hash());
        let (store, chain) = make_store_with_chain_starting_from(5, genesis);

        let result = request_range(
            store,
            NetworkPoint::Specific(Slot::from(2), run_strategy(any_header()).hash()),
            chain[3].header.point(),
        );
    ...
    fn test_request_range_slot_before_from_abort() {
        ...
        let non_existent_hash = run_strategy(any_header()).hash();
