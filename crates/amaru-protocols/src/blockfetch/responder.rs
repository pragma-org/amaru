...
impl PointsRange {
    ...
    pub fn request_range(
        ...
        if store.load_from_best_chain(&through).is_some() {
            ...
            tracing::debug!(%from, "requested 'from' point is not on the best chain. It might be on a fork");

            if let Some(points) = store.ancestors_points(&from, &through) {
                Some(PointsRange::Fork(points))
            } else {
                tracing::debug!(%from, %through, "the requested range is not on a valid fork");
                None
            }
        }
    }
