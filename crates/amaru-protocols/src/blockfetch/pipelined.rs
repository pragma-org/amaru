...
use amaru_pure_stage::{DeserializerGuards, Effects, StageRef, define_role, err, typestate::prelude::*};

use super::{
    Blocks, Message,
    responder::MAX_FETCHED_BLOCKS,
    typestate::{
        self, BatchDone, Block, Busy, ClientDone, Done, Idle, Live, NoBlocks, RequestRange, StartBatch, ToCollector,
        ToResponder,
    },
};
use crate::{
...
/// Node request as seen by a pipelined instance (distinct from the lock-step experiment `Fetch`).
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct Fetch {
...
/// Distinct from [`typestate::Close`] so `OnReceive` impls do not collide.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct Close;
...
impl Instance {
    fn new(peer: Peer) -> Self {
        Self { live: initial_state::<Idle>().into(), inflight: None, peer }
    }

    async fn on_fetch<M: std::marker::Send + 'static>(
        &mut self,
        fetch: Fetch,
        eff: &Effects<M>,
    ) -> anyhow::Result<Step> {
        ...
        let session = idle.receive(fetch, eff.clone()).send(&dest, range.clone()).await;
        self.live = session.finish().into();
        Ok(Step { wire: Some(range.into()), credit: Some(SwitchCredit::Left) })
    }

    async fn on_close<M: std::marker::Send + 'static>(&mut self, eff: &Effects<M>) -> anyhow::Result<Step> {
        let Some(idle) = take_idle(&mut self.live) else {
    ...
    async fn on_network<M: std::marker::Send + 'static>(
        &mut self,
        msg: Message,
        eff: &Effects<M>,
    ) -> anyhow::Result<Step> {
        match msg {
            Message::StartBatch => {
                ...
                self.live = busy.receive(StartBatch, eff.clone()).finish().into();
            ...
            Message::NoBlocks => {
                ...
                let session = busy
                    ...
                    .send_any(&collector, Blocks::NoBlocks(inflight.id, self.peer.clone()))
                    .await;
                self.live = session.finish().into();
            ...
            Message::Block { body } => {
                ...
                let session = st
                    ...
                    .send_any(&collector, Blocks::Block(id, self.peer.clone(), network_block))
                    .await;
                self.live = session.finish().into();
                Ok(Step { wire: None, credit: Some(SwitchCredit::Stay) })
            }
            Message::BatchDone => {
                ...
                let collector = CollectorOut::new(inflight.cr);
                let session = st.receive(BatchDone, eff.clone()).send_any(&collector, Blocks::Done(inflight.id)).await;
                self.live = session.finish().into();
                Ok(Step { wire: None, credit: Some(SwitchCredit::Entered) })
            }
...
fn take_streaming(live: &mut Live) -> Option<typestate::Streaming> {
...
impl Handler {
    fn new(n: NonZeroUsize, muxer: StageRef<MuxMessage>, peer: Peer) -> Self {
        Self {
            ...
            instances: (0..n.get()).map(|_| Instance::new(peer.clone())).collect(),
            muxer,
        }
    }
}
...
pub fn handler() -> impl Fn(
    Handler,
    Inputs<super::BlockFetchMessage>,
    Effects<Inputs<super::BlockFetchMessage>>,
) -> amaru_pure_stage::BoxFuture<'static, Handler>
+ std::marker::Send
+ 'static {
    |state, msg, eff| Box::pin(handle(state, msg, eff))
}
...
async fn admit_fetch(state: &mut Handler, fetch: Fetch, eff: &Effects<Inputs<super::BlockFetchMessage>>) {
    match state.pipeline.try_admit(fetch) {
        ...
        Admit::ReplacedSlack(_) => {
            tracing::debug!("dropping request for slow peer");
        }
        Admit::Dropped => {}
    }
}
...
async fn on_from_network(state: &mut Handler, bytes: NonEmptyBytes, eff: &Effects<Inputs<super::BlockFetchMessage>>) {
    ...
    if matches!(msg, Message::RequestRange { .. } | Message::ClientDone) {
        tracing::debug!("initiator variant");
...
pub async fn register_blockfetch_initiator_pipelined<M: amaru_pure_stage::SendData>(
    ...
    eff.send(
        ...
        MuxMessage::Register {
            ...
            handler: eff.contramap(&blockfetch, "blockfetch_pipeline_bytes", Inputs::Network).await,
            max_buffer: blockfetch_pipeline_max_buffer(n),
        },
    )
    .await;
    eff.contramap(&blockfetch, "blockfetch_pipeline_local", Inputs::Local).await
}

#[cfg(test)]
mod tests {
    use amaru_kernel::Point;
    use amaru_pure_stage::{StageGraph, simulation::SimulationBuilder};
