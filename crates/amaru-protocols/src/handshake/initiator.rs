        ...
        async move {
            Ok(match input {
                InitiatorResult::Propose => {
                    debug!(protocols::handshake::initiator::PROPOSING_VERSIONS, our_versions = ?self.our_versions);
                    (Some(InitiatorAction::Propose(self.our_versions.clone())), self)
                }
                InitiatorResult::Conclusion(handshake_result) => {
                    debug!(protocols::handshake::initiator::CONCLUSION, handshake_result = ?handshake_result);
                    eff.send(&self.connection, handshake_result).await;
                    (None, self)
                }
                InitiatorResult::SimOpen(version_table) => {
                    debug!(protocols::handshake::initiator::SIMULTANEOUS_OPEN, version_table = ?version_table);
