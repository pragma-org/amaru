...
async fn test_connect_initiator_reconnection() -> anyhow::Result<()> {
    ...
    let (initiator, initiator_done) = start_initiator_at_with_configuration(
...
async fn test_connect_initiator_reconnection_on_responder_restart() -> anyhow::Result<()> {
    ...
    let responder_configuration = Configuration::responder()
        .with_addr(addr)
        .with_slow_manager();
    let responder = start_responder_with_configuration(responder_configuration.clone())
        .await?
        .0;
    let (initiator, initiator_done) = start_initiator_at_with_configuration(
        Configuration::initiator()
    ...
    // Both the initiator and the responder should eventually terminate with the same stater
...
async fn start_initiator_at(
    ...
    start_initiator_at_with_configuration(Configuration::initiator().with_addr(addr)).await
}

/// Create and start an initiator node that connects to the given port.
/// ChainSync events are sent to a stage that stores them in the in-memory store without further processing.
async fn start_initiator_at_with_configuration(
    configuration: Configuration,
) -> anyhow::Result<(TokioRunning, Arc<Notify>)> {
