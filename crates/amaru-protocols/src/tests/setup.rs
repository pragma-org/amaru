...
pub(super) fn set_resources_with_connections(
    ...
    let block_validation = Arc::new(MockBlockValidator::new(chain_store.get_best_chain_tip()));
    ...
    network.resources().put::<ResourceMempool<WithOriginalBytes<Transaction>>>(mempool);
    Ok(())
}
