...
impl Manager {
    ...
    async fn connection_result(
        ...
        match result {
            ...
            Err(err) => {
                info!(protocols::manager::peer::CONNECT_FAILED, peer = &peer, error = ?err);
    ...
    async fn fetch_blocks(
        ...
        debug!(protocols::manager::blocks::FETCH, from = from, through = through, peers = ?peers);
...
async fn close_connection(eff: &Effects<ManagerMessage>, peer: &Peer, conn_id: ConnectionId) {
    if let Err(err) = Network::new(eff).close(conn_id).await {
        error!(protocols::manager::peer::CLOSE_FAILED, peer = peer, error = ?err);
    }
}
