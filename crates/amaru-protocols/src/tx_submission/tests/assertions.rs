...
pub fn expect_transactions(mempool: Arc<dyn TxSubmissionMempool<Transaction>>, txs: Vec<Transaction>) {
    ...
    if actual.len() != txs.len() {
        panic!(
            ...
            actual.iter().map(|id| format!("{}", id)).collect::<Vec<_>>().join(",\n"),
...
#[track_caller]
pub fn assert_actions_eq<T: std::fmt::Debug + std::fmt::Display + PartialEq>(actual: &[T], expected: &[T]) {
    if actual != expected {
        panic!(
            "\nActual outcomes\n\n{}\n\nExpected outcomes\n\n{}\n",
            actual.iter().map(|id| format!("{:#?}", id)).collect::<Vec<_>>().join(",\n"),
            expected.iter().map(|id| format!("{:#?}", id)).collect::<Vec<_>>().join(",\n")
        );
    }
}
