...
pub use responder::{MEMPOOL_INSERT_TIMEOUT, ResponderLocalIn, ResponderResult, TxSubmissionMsg, responder};
#[cfg(test)]
pub use tests::*;
...
pub async fn register_tx_submission(
    ...
    let tx_submission = if role == Role::Initiator {
        ...
        let (state, stage) = responder::TxSubmissionResponder::new(muxer.clone(), params, origin, mempool_stage);
