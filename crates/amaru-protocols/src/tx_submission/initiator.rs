...
impl ProtocolState<Initiator> for State {
    ...
    fn network(&self, input: Self::WireMsg) -> anyhow::Result<(Outcome<Self::WireMsg, Self::Out, Self::Error>, Self)> {
        let _span = debug_span!(
            protocols::tx_submission::initiator::TX_SUBMISSION_INITIATOR_PROTOCOL,
...
impl TxSubmissionInitiator {
    ...
    async fn request_tx_ids_blocking(
        ...
        debug!(protocols::tx_submission::initiator::WAIT_FOR_AT_LEAST, peer = self.peer, seq_no = expected_seq_no.0);
        debug_record!(protocols::tx_submission::initiator::WAIT_FOR_AT_LEAST, req = req);
        eff.send(
            &self.mempool_stage,
    ...
    fn emit_reply_tx_ids(&self, tx_ids: &[(TransactionId, u32)]) {
        debug!(
            ...
            peer = self.peer,
            count = tx_ids.len(),
            tx_ids = display_collection(tx_ids.iter().map(|(id, _)| id.short()))
        );
    }
