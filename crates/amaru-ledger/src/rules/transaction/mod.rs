...
mod tests {
    ...
    use amaru_kernel::{
        EraHistory, Hash, ProtocolParameters, Transaction, cbor, json, utils::serde::FilesystemRefResolver,
    };
    use amaru_plutus::arena_pool::ArenaPool;

    use super::fixture::{Expected, Fixture, Predicate};
    use crate::{
        context::{DefaultValidationContext, PoolVrfs},
    ...
    fn run_conformance(fixture_path: &str) {
        ...
        // Fixtures identify pools by id alone, so conjure each a deterministic placeholder VRF
        // key from its id; the in-use set is derived from the pools, consistent by construction.
        let pools = fixture
            .initial_state
            .pools
            .into_iter()
            .map(|pool_id| {
                let mut vrf = [0; 32];
                vrf[..28].copy_from_slice(pool_id.as_ref());
                (pool_id, PoolVrfs { current: Hash::new(vrf), pending: None })
            })
