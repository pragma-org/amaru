    ...
    C: UtxoSlice,
{
    ...
    for collateral in collaterals.iter() {
        ...
        parse_address(output).and_then(|address| {
            if address.has_script() {
                Err(InvalidCollateral::LockedAtScriptAddress(collateral.into()))
            } else {
                Ok(())
            }
        })?;
    ...
    let collateral_return_balance = match collateral_return {
        Some(output) => get_minted_output_balance(output),
...
fn parse_address(output: &TransactionOutput) -> Result<Address, InvalidCollateral> {
...
fn add_value_to_balance(balance: &mut CollateralBalance, output: &TransactionOutput) {
    let output_balance: CollateralBalance = get_output_balance(output);
...
fn get_output_balance(output: &TransactionOutput) -> CollateralBalance {
    match output {
        amaru_kernel::PseudoTransactionOutput::Legacy(legacy) => (&legacy.amount).into(),
        amaru_kernel::PseudoTransactionOutput::PostAlonzo(modern) => (&modern.value).into(),
    }
}

fn get_minted_output_balance(output: &MintedTransactionOutput<'_>) -> CollateralBalance {
    match output {
        amaru_kernel::PseudoTransactionOutput::Legacy(legacy) => (&legacy.amount).into(),
        amaru_kernel::PseudoTransactionOutput::PostAlonzo(modern) => (&modern.value).into(),
    }
