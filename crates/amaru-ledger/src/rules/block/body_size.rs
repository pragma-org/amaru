use crate::rules::RuleViolation;
use amaru_kernel::{cbor, protocol_parameters::ProtocolParameters, MintedBlock};
pub struct BlockBodySizeTooBig {
    pub supplied: usize,
    pub max: usize,
}

impl From<BlockBodySizeTooBig> for RuleViolation {
    fn from(value: BlockBodySizeTooBig) -> Self {
        RuleViolation::BlockBodySizeTooBig(value)
    }
}

//FIXME: We are re-serializing here, which is dangerous and should not be done.
// We should instead work with the raw CBOR and esnure the calculations are the same as the Haskell implementation
pub fn block_body_size_valid(
    block: &MintedBlock<'_>,
    protocol_params: &ProtocolParameters,
) -> Result<(), BlockBodySizeTooBig> {
    let max_block_size = protocol_params
        .max_block_body_size
        .try_into()
        .unwrap_or_else(|_| panic!("Failed to convert u32 to usize"));

    let mut tx_bodies_raw = Vec::new();
    if cbor::encode(block.transaction_bodies.clone(), &mut tx_bodies_raw).is_err() {
        // TODO: handle appropriately
        panic!("failed to encode block transaction_bodies in CBOR");
    }

    let mut tx_witness_sets_raw = Vec::new();
    if cbor::encode(
        block.transaction_witness_sets.clone(),
        &mut tx_witness_sets_raw,
    )
    .is_err()
    {
        // TODO: handle appropriately
        panic!("failed to encode block transaction_witness_sets in CBOR");
    }

    let mut auxiliary_data_set_raw = Vec::new();
    if cbor::encode(
        block.auxiliary_data_set.clone(),
        &mut auxiliary_data_set_raw,
    )
    .is_err()
    {
        // TODO: handle appropriately
        panic!("failed to encode block auxiliary_data_set in CBOR");
    }

    let mut invalid_tranasctions_raw = Vec::new();
    if cbor::encode(
        block.invalid_transactions.clone(),
        &mut invalid_tranasctions_raw,
    )
    .is_err()
    {
        // TODO: handle appropriately
        panic!("failed to encode block invalid_transactions in CBOR");
    }

    let block_body_size = tx_bodies_raw.len()
        + tx_witness_sets_raw.len()
        + auxiliary_data_set_raw.len()
        + invalid_tranasctions_raw.len();

    if block_body_size > max_block_size {
        Err(BlockBodySizeTooBig {
            supplied: block_body_size,
            max: max_block_size,
        })
    } else {
        Ok(())
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_block_body_size_valid() {
        // These bytes are Conway3.block from Pallas https://github.com/txpipe/pallas/blob/main/test_data/conway3.block
        let bytes = hex::decode("").expect("Failed to decode Conway3.block hex");

        let (_, block): (u16, MintedBlock<'_>) =
            cbor::decode(bytes.as_slice()).expect("Failed to parse Conway3.block bytes");

        let pp = ProtocolParameters::default();
        assert!(block_body_size_valid(&block, &pp).is_ok())
    }
}
