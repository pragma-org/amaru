...
pub fn block_header_size_valid(
    ...
    let max_header_size = protocol_params
        ...
        .try_into()
        .unwrap_or_else(|e| unreachable!("failed to convert u32 to u64 ({e:?})! Architecture is less than 32-bit ?!"));

    if actual_header_size > max_header_size {
        Err(InvalidBlockDetails::HeaderSizeTooBig { supplied: actual_header_size, max: max_header_size })
    } else {
        Ok(())
    }
}
