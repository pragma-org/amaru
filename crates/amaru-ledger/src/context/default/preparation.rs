...
use amaru_kernel::{
    DRep, DRepRegistration, MemoizedTransactionOutput, PoolId, ProposalEnum, ProposalId, ProposalsRoots,
...
impl<'block> DefaultPreparationContext<'block> {
    pub fn into_validation_context<'volatile>(
        ...
        volatile: &'volatile impl VolatileState<
            ...
            Proposal = Existence<ProposalEnum>,
        >,
        db: &impl ReadStore,
...
pub fn resolve_proposals(
    volatile: &impl VolatileState<Proposal = Existence<ProposalEnum>>,
    db: &impl ReadStore,
    mut keys: impl Iterator<Item = ProposalId>,
) -> Result<BTreeMap<ProposalId, ProposalEnum>, ContextHydratationError> {
            ...
            match volatile.resolve_proposal(&id) {
                ...
                Existence::Unknown => {
                    if let Some(row) = db.proposal(&id).map_err(ContextHydratationError::ResolveProposals)? {
                        from_db += 1;
                        proposals.insert(id, ProposalEnum::from(&row.proposal.gov_action));
                    }
