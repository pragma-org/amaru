...
impl PotsSlice for AssertValidationContext {
    fn add_fees(&mut self, fee: Lovelace) {
        let _span = trace_span!(ledger::context::ADD_FEES, fee = fee);
...
impl WitnessSlice for AssertValidationContext {
    fn require_vkey_witness(&mut self, vkey_hash: Hash<28>) {
        let _span = trace_span!(ledger::context::REQUIRE_VKEY_WITNESS, hash = format!("{}", vkey_hash));
        let _guard = _span.enter();
    ...
    fn require_script_witness(&mut self, script: RequiredScript) {
        let _span = trace_span!(ledger::context::REQUIRE_SCRIPT_WITNESS, hash = format!("{}", script.hash));
        let _guard = _span.enter();
    ...
    fn require_bootstrap_witness(&mut self, root: Hash<28>) {
        let _span =
            trace_span!(ledger::context::REQUIRE_BOOTSTRAP_WITNESS, bootstrap_witness_hash = format!("{}", root));
        let _guard = _span.enter();
