...
use crate::{
    ...
    state::{
        BackwardError, EVENT_TARGET, MIN_LEDGER_SNAPSHOTS, StakeDistributionView, StateError, ValidationContextError,
        compute_stake_distribution, initial_stake_distributions,
        overlay::StateOverlay,
        unsafe_slot_in_epoch, unsafe_slot_to_epoch,
    ...
    summary::{rewards::RewardsSummary, stake_distribution::StakeDistribution},
};

/// The `StateSnapshot` holds the ledger volatile values and some handles to the stable store.
/// `StateSnapshots` values can be cheaply cloned in order to be mutated (via methods like `push_fragment`, `rollback_to`, etc...).
///
/// The state itself is split into two sub-components:
///
...
///
pub struct StateSnapshot<S, HS> {
    /// A handle to the stable store, shared across all ledger instances.
    stable: Arc<Mutex<S>>,

    /// A handle to the historical snapshot stores, shared across all instances.
    /// Wrapped in `Arc` so that cloning a `StateSnapshot` does not require `HS: Clone`.
    snapshots: Arc<HS>,
...
impl<S: Store, HS: HistoricalStores> Clone for StateSnapshot<S, HS> {
    fn clone(&self) -> Self {
        Self {
...
impl<S: Store, HS: HistoricalStores> Deref for StateSnapshot<S, HS> {
...
impl<S: Store, HS: HistoricalStores> DerefMut for StateSnapshot<S, HS> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.overlay
    }
}

impl<S: Store, HS: HistoricalStores> StateSnapshot<S, HS> {
    pub fn new(
        stable: S,
        snapshots: HS,
        network: NetworkName,
    ...
    /// Network this ledger is connected to.
    pub fn network(&self) -> NetworkName {
        self.network
    }

    /// Era history for the network.
    pub fn era_history(&self) -> &EraHistory {
        &self.era_history
    }

    /// Non-updatable global parameters (slot length, epoch length, security parameter, …).
...
impl<S: Store, HS: HistoricalStores> StateSnapshot<S, HS> {
    ...
    fn apply_block(&mut self, now_stable: AnchoredVolatileFragment) -> Result<(), StateError> {
        ...
        if self.overlay.epoch() == tip_epoch && self.is_empty() {
