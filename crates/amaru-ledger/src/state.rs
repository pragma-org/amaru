...
impl<S: Store> State<S> {
    ...
    fn apply_block(&mut self, now_stable: AnchoredVolatileState) -> Result<(), StateError> {
        ...
        let current_epoch = self
            .era_history
            .slot_to_epoch(slot_arithmetic::Slot::new(start_slot))
        ...
        let epoch = self
            .era_history
            .slot_to_epoch(slot_arithmetic::Slot::new(anchor_slot))
    ...
    pub fn forward(&mut self, next_state: AnchoredVolatileState) -> Result<(), StateError> {
        ...
        let relative_slot = self
            .era_history
            .slot_in_epoch(slot_arithmetic::Slot::new(next_state_slot))
            .map_err(|e| StateError::ErrorComputingEpoch(next_state_slot, e))?;

        if self.rewards_summary.is_none()
            && relative_slot >= slot_arithmetic::Slot::new(STABILITY_WINDOW as u64)
        {
...
impl HasStakeDistribution for StakeDistributionView {
    ...
    fn get_pool(&self, slot: Slot, pool: &PoolId) -> Option<PoolSummary> {
        ...
        let epoch = self
            .era_history
            .slot_to_epoch(slot_arithmetic::Slot::new(slot))
            .ok()?
            - 2;
