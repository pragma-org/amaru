...
pub fn tm_resume() -> TraceMatch<'static> {
    TraceMatch::Property(Box::new(|src| matches!(src.entry(), Some(TraceEntry::Resume { .. }))), "Resume".to_string())
}
...
pub fn tm_resume_external_match<'a, T: SendData>(
    ...
    TraceMatch::Property(
        Box::new(move |src| {
            let Some(TraceEntry::Resume { stage, response: StageResponse::ExternalResponse(response) }) = src.entry()
            else {
                return false;
            };
            if stage != &stage_name {
