...
impl SimulationRunning {
    ...
    ///
    /// World loops that complete `UntilResolved` network effects keep the typed result in a
    /// `Box<dyn SendData>`; wrapping that again would fail deserialization.
    pub fn complete_external_box(&mut self, at_stage: impl AsRef<Name>, result: Box<dyn SendData>) {
        let at_stage = at_stage.as_ref().clone();
        {
            let data = self.stages.get_mut(&at_stage).unwrap_or_else(|| panic!("stage `{at_stage}` does not exist"));
            let data = data.assert_stage("which cannot receive external effects");
