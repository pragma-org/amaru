...
- **amaru-pure-stage**: `Session::discard_repeat` drops a leading `Repeat` so the suffix can be sent. `send` cannot skip a star by itself: `Repeat<Send<Role, T>>` unifies `T` with the repeated payload.
...
- **amaru-protocols**: BlockFetch initiator and responder rewritten as typestate machine. Pipeline depth is a `NonZeroU8`; `N = 1` drives the lock-step instance and `N > 1` wraps it in the CIP-0164 pipeliner. The responder serves a range as `StartBatch`, `Repeat<Call<Block>>`, `BatchDone` in one remainder. Wire payloads are `Call`s on the mux (awaiting `Sent`) so TCP back-pressure reaches the handler; `WantNext` stays a `Send`.
