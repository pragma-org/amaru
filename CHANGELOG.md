...
- **amaru-protocols**: BlockFetch initiator and responder rewritten as typestate machine. Pipeline depth is a `NonZeroU8`; `N = 1` drives the lock-step instance and `N > 1` wraps it in the CIP-0164 pipeliner. The responder serves a range as `StartBatch`, `Repeat<Send<Block>>`, `BatchDone` in one remainder.
