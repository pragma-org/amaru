...
start() {
  ...
  # Layout:
  # nodes window: left = upstream, right split top/bottom = sut/down
  tmux split-window -t "$SESSION:nodes" -h -c "$AMARU_DIR"     # create right pane
  tmux split-window -t "$SESSION:nodes.1" -v -c "$AMARU_DIR"   # split right into sut/down

  tmux select-pane -t "$SESSION:nodes.0" \; select-pane -t "$SESSION:nodes.1"

  # Name panes
  tmux select-pane -t "$SESSION:nodes.0" -T "upstream"
  tmux select-pane -t "$SESSION:nodes.1" -T "amaru"
  tmux select-pane -t "$SESSION:nodes.2" -T "amaru-downstream"
  ...
  pane_run "$SESSION:nodes.1" "$(cmd_amaru)"
  pane_run "$SESSION:nodes.2" "$(cmd_amaru_downstream)"
...
restart() {
  ...
  case "$pane" in
    ...
    amaru)             pane_run "$SESSION:nodes.1" "$(cmd_amaru)" ;;
    amaru-downstream)  pane_run "$SESSION:nodes.2" "$(cmd_amaru_downstream)" ;;
