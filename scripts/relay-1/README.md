...
| `CARDANO_UPSTREAM_MODE`                | No       | `public` on mainnet, `local` otherwise                                                                                                                                                                                                                             | Use a local cardano-node or a public upstream peer                       |
| `PUBLIC_UPSTREAM_PEER_ADDRESS`         | No       | `backbone.cardano.iog.io:3001`                                                                                                                                                                                                                                     | Public Cardano peer used when `CARDANO_UPSTREAM_MODE=public`            |
| `TX_QUERY_SOURCE`                      | No       | `koios` when using public upstream, `local` otherwise                                                                                                                                                                                                               | Source for UTxOs, protocol parameters, and upstream tip                  |
| `KOIOS_API_URL`                        | No       | `https://api.koios.rest/api/v1`                                                                                                                                                                                                                                    | Koios API base URL used when `TX_QUERY_SOURCE=koios`                    |
...
| `TX_PAYMENT_SKEY`                      | No       | First existing path among `scripts/relay-1/keys/$AMARU_NETWORK/payment.skey`, `scripts/relay-1/run/$AMARU_NETWORK-wallet/payment.skey`, `scripts/relay-1/keys/payment.skey`                                                                                       | Payment signing key for runtime tx generation                            |
...
To generate fresh transactions at runtime, the script uses a network-specific payment signing key by default. It first
checks `scripts/relay-1/keys/$AMARU_NETWORK/payment.skey`, then
`scripts/relay-1/run/$AMARU_NETWORK-wallet/payment.skey`, then falls back to `scripts/relay-1/keys/payment.skey`. You can
override it with another funded payment signing key.
The script derives the address from it, queries either the local upstream Haskell node or Koios for UTxOs at that
address, builds up to `TX_GENERATED_COUNT` independent self-transfer transactions with 1 ada outputs,
