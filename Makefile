...
import-snapshots: download-snapshots ## Import snapshots
...
bootstrap: import-headers import-snapshots import-nonces ## Bootstrap the node
...
run: enforce-peer-address ## Run the node
	AMARU_DEV_LOG=error cargo run --release -- --with-json-traces daemon --peer-address=${AMARU_PEER_ADDRESS} ${AMARU_RUN_EXTRA}